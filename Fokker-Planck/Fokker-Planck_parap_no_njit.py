...
def M_v(v):
    return (1/np.sqrt(2*np.pi))*np.exp(-(v**2)/ (2*diff_coef))
...
def job(xind):
    ...
    for vind in range(numv):
        if xind == 0 or xind==numx-1 or vind == 0 or vind==numv-1:
            ...
            densityxv[vind] = diff_coef*gamma*step_time*liouvilian+density0[xind][vind]-step_time*(TRANSP1+TRANSP2+TRANSP3+TRANSP4)
    return densityxv
...
for tind in range(numt-1):
    ...
    print('T : '+str(tind*step_time)+' : %1.6f'%(normalisation_time[tind])+'     T(s) : %1.6f'%(elapsed_time)+'     max : %1.6f'%(max(density_integrated_v[tind])))
